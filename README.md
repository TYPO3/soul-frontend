...
| [`<sds-note>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-note) | What an answer carries besides the answer. `tone` (`info`, `ok`, `warn`, `error`), `heading`, `body`, `icon`, `label` |
| [`<sds-table>`](https://typo3.github.io/soul-design-system/frontend/components/data.html#component-sds-table) | Rows and columns, with the scroll a wide one needs. `density` (`compact`, `medium`, `airy`), `scrollable`, `width`, `.columns`, `.rows` |
...
| [`<sds-search>`](https://typo3.github.io/soul-design-system/frontend/components/navigation.html#component-sds-search) | Finding a page in a site that has no server: the index is fetched on the first keystroke and the hits are drawn under the field. `index`, `label` |
| [`<sds-result>`](https://typo3.github.io/soul-design-system/frontend/components/navigation.html#component-sds-result) | One hit in a list of them. `heading`, `href`, `path`, `snippet`, `match` for what was searched for inside it, `kind`, `meta` |
