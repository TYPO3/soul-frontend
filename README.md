...
| [`<sds-surface>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-surface) | A filled plane holding a statement. `plane` (`raised`, `sunken` for machine output), `heading`, `body`, `label`, `icon`, `box-style` sizes the plane where one instance needs it — nothing by default, the element fills the cell a wall stretches for it |
