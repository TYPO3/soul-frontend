...
| [`<sds-slide>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-slide) | One 16:9 frame of a deck, the page at twice the size. Every element between the tags keeps its set. `kind` is `cover`, `section`, `statement`, `content` or `closing`. `ground` is `paper` or `terminal`. `eyebrow`, `heading`, `lead`, `note`, `number`. The lockup as `signet`, `brand`, `product`. The outline as `sections` and `current`. `fit` scales the frame to the room it has |
