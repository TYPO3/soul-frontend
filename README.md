...
| [`<sds-slide>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-slide) | One 16:9 frame of a deck, the page at twice the size. Every element between the tags keeps its set. `kind` is `cover`, `speaker`, `section`, `statement`, `content`, `closing` or `figure`, which gives a page's material the frame. A speaker's `portrait` and `alt`. `ground` is `paper` or `terminal`. `eyebrow`, `heading`, `lead`, `note`, `number`. The lockup as `signet`, `brand`, `product`. The outline as `sections` and `current`. `fit` scales the frame to the room it has, and `shrink` makes it a picture at its column's width. `zoomable` opens it in the deck, and a press anywhere does the same. What it holds shrinks until it fits, and takes no press |
| [`<sds-deck>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-deck) | Slides one after the other at the window's size. Slides between the tags are a deck of its own. The page shows its cover, a play and a PDF press. With none, it runs through the page's slides, `from` an id. `label`. The lockup once as `brand`, `product`, `signet`, `signet-large`, and `numbered` counts the slides; a slide's own wins. Keys, drag, a list of pictures, the full screen, and the print as PDF. `show(at)`, `go(index)`, `close()`, `fullscreen()`, `print()` |
...
| [`<sds-timeline>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-timeline), [`<sds-timeline-stop>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-timeline) | A plan on the calendar: dated stops down one rail. The stops between the tags as `sds-timeline-stop`, each with `when` as the plan writes it and `heading`, one with `now`. What a stop holds goes between its tags, and the stops inside it as more of these. Or as `.entries` of `{ when, heading, body?, now?, items? }`, where a page holds the plan as data. The plan writes `state` onto each stop. The passed ones carry the check, the one now a filled disc and the word, the ones ahead a ring. Not an instruction and not a run |
| [`<sds-decision>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-decision), [`<sds-answer>`](https://typo3.github.io/soul-design-system/frontend/components/content.html#component-sds-decision) | The question a paper asks: `question`, `lead`, `by`, `due`, `label`. The answers between the tags as `sds-answer`, with `key`, `heading`, `recommended` and what it means between its tags. Or as `.answers`, where a page holds them as data |
| [`<sds-compare>`](https://typo3.github.io/soul-design-system/frontend/components/media.html#component-sds-compare) | Two pictures side by side, each a figure with its claim. `before-src`, `before-alt`, `before-caption` and the same for `after`; `before-label` and `after-label` are the words over them; `zoomable` opens either |
...
| [`<sds-table>`](https://typo3.github.io/soul-design-system/frontend/components/data.html#component-sds-table) | Rows and columns, with the scroll a wide one needs. `density` (`compact`, `medium`, `airy`), `scrollable`, `width`, `caption`, `caption-side` (`bottom`, `top`), `.columns`, `.rows`. A cell is text, a component, or `{ value, note }` for a name with what is true about it under it. `loading` with `loading-rows` draws bars at the row height |
...
| [`<sds-nav-outline>`](https://typo3.github.io/soul-design-system/frontend/components/navigation.html#component-sds-nav-outline) | The parts of a long document, beside it in the panel of `sds-paper`. The whole tree, `numbered` from each entry's place as text in the row. The mark is on the part the reader is in. On a phone it folds behind one press in the page's head |
...
shell: `sds-shell`, `sds-body`, `sds-column`, `sds-bands`, `sds-page`, and
`sds-paper`, the panel beside a document that goes out on its own. The
