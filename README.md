...
| [`<sds-table>`](https://typo3.github.io/soul-design-system/frontend/components/data.html#component-sds-table) | Rows and columns, with the scroll a wide one needs. `density` (`compact`, `medium`, `airy`), `scrollable`, `width`, `.columns`, `.rows`, and `loading` with `loading-rows` for the wait before them — the head stays and the body is drawn as bars at the height the rows will have |
