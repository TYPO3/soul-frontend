/* sds-dialog — a modal that is actually a dialog.
   ...
   still picture and has nothing to open. This is the behaviour, on the
   platform's `<dialog>`: the open, the focus taken and given back, Escape,
   the page behind it inert.

   Deliberately not one component. A dialog that must open to draw is one no
   card can document. A surface that grabs the focus is no use in a
   specimen. */
...
export class SdsDialog extends SdsElement {
  ...
  protected override updated(): void {
    ...
    /* `showModal()` throws if the element is not in the document, and on the
       first update it need not be yet. An exception here breaks the update
       cycle and the component renders nothing at all. */
    if (!this.isConnected) return;

    try {
      /* Cleared at every open: `returnValue` outlives a close, and a second
         question dismissed with Escape otherwise reports the first one's
         answer. */
  ...
  protected override render(): TemplateResult {
    /* `<dialog>` carries the semantics; the `sds-modal` classes carry the
       look. One surface, described once in `components.css`, in a specimen
       and in a product alike. */
    return html`<dialog
  ...
  private foot(): TemplateResult {
    ...
    /* A glyph is a sibling of the label rather than inside it. The row's gap
       sets the two apart, and `<sds-icon>` with no name throws. */
