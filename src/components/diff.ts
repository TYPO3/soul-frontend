/* sds-diff — a file's changes, and the one place status colour fills a line.
   ...
   The tint is 14%, so a changed line reads as changed and the row stays quiet.
   No line numbers unless something references them. A gutter of numbers
   nobody cites is decoration on the one surface with the least room for it.
...
export interface DiffProps {
  ...
  path: string;
  /** A glyph beside the path, where the kind of file deserves a word before
      the change. */
  icon?: IconId;
