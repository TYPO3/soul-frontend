/* What `sds-icon` looks like once there is no browser to resolve it.
   ...
   Beside the element, not inside the renderer: `render.ts` must no more know
   the shape of an icon than the shape of a button. */
...
/* A reference into a sprite, and only into a sprite. The browser reference
   ...
   rather than left to the run order. A referenced drawing has the same `<use
   href="…#soul-ref">` shape, and the lookup takes it for an icon. */
...
/** The package ships each glyph pretty-printed over several lines. Collapsed
    ...
    generated card pixel-match its baseline. The shapes expand because a card
    parses as HTML, where a self-closing tag on a non-void element does not
    close. */
